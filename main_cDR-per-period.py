...
def output_DR(param_sets, PCP_id):
    ...
    ### Required parameters.
    if PCP_id == 'TCP':
        t1, t2, veloc, lam1, lam2p, mb, sigma, P1, P2, beta = param_sets
    elif PCP_id == 'MCP':
        t1, t2, veloc, lam1, lam2p, mb, rd, P1, P2, beta = param_sets
    else:
...
#    DivNum_x_type1, DivNum_x_type2 = 100, 20
    ...
    DivNum_r = 200
    DivNum_z = 200
    DivNum_x_type1, DivNum_x_type2 = 200, 200
    ...
    start1 = time.time()
    DR_term1 = summand_DR_term1(t1)
    end1 = time.time()
    e_time1 = end1 - start1

    start2 = time.time()
    DR_term2 = summand_DR_term2(t2)
    end2 = time.time()
    e_time2 = end2 - start2

    return [t1, t2, DR_term1, DR_term2], [e_time1, e_time2]
...
if __name__ == "__main__":
    ...
    ## Modify the modified matrix.
    def modify_matrix(matrix_pre):
        ## check if all the rows in the matrix are the same.
        def check_correspondence(matrix):
            check_flag = True
            row_first = matrix[0]
            for i in range(len(matrix)):
                row_check = matrix[i]
                if set(row_first) ^ set(row_check):    ## if there exists difference between the two lists
                    check_flag = False
                else:
                    pass
            return check_flag

        row_s_ppp = np.array(matrix_pre)[:, 0].tolist()
        row_s_pcp = np.array(matrix_pre)[:, 1].tolist()
        matrix_other_params = np.array(matrix_pre)[:, 2:].tolist()

        row_s_ppp, row_s_pcp = list(map(float, row_s_ppp)), list(map(float, row_s_pcp))
        max_s = np.max(row_s_ppp + row_s_pcp)    # The maximum among s1 and s2 in input csv.
        if check_correspondence(matrix_other_params):
            row_other_params = matrix_other_params[0]
        else:
            print('Error, modify_matrix: all the other parameters except s1,s2 must be the same for each row in input csv.')
            sys.exit()

        matrix_mod = []
        for t1, t2 in zip(np.arange(max_s + 1.0), np.arange(max_s + 1.0)):
            row_mod = [t1, t2] + row_other_params
    ...
    ## Modify the matrix (make the column of t1:0~s_ppp_max and t2:0~s_pcp_max)
    matrix_mod = modify_matrix(matrix)

    ## The matrix data of the result csv.
    result_rows_L = []
    NoticeMessagesFlag = False
    for i in range(len(matrix_mod)):
        ParamSets, CurveType, ErrorFlag = make_parameters_set(matrix_mod[i])
        if not ErrorFlag:
            #result_row1, result_row3 = output_DR(ParamSets, 'MCP', flag_sobol=True)   ## temporary script (need modification !!!)
        ...
        if not i:
            ...
            result_row2 = np.sum(np.array(result_rows_L)[:, 2:4], axis=0).tolist()
        result_rows_L.append(result_row1 + result_row2 + result_row3)

        ### test start ###
        print('i = {}, {}'.format(i, result_row1 + result_row2 + result_row3))
        ### test end ###
    ...
    header1 = ['instantaneous time1 (macro)', 'instantaneous time2 (small)', 'instantaneous data rate1 (macro)', 'instantaneous data rate2 (small)']
    header2 = ['sum data rate1 (macro)', 'sum data rate2 (small)']
    header3 = ['elapsed time1 (macro)', 'elapsed time2 (small)']
    result_header = header1 + header2 + header3


    ## Output the result csv file.
    OutputCsvFilename = 'datacsv_DR-per-period-{}_'.format(PCP_id) + InputCsvFilename
    f = open('database/'+OutputCsvFilename, 'w')
